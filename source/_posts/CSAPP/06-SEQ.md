...
{% note info '硬件图例解码 (Reading the Diagram)' %}
...
{% note info 'SEQ 数据通路核心信号 (Key Signals)' %}
...
{% note default '**为什么需要两个写端口 (`dstE` 和 `dstM`)？**' %}
...
{% note info 'SEQ 控制信号总览 (Control Signals Overview)' %}
...
{% note danger 'SEQ 的致命弱点：为什么我们需要流水线？' %}
