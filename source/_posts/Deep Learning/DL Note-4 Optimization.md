...
假设函数 $J(\theta)$ 是凸函数、可微，且梯度满足 $L$-Lipschitz 连续条件。对于梯度下降法，可以证明其收敛速率为： 

$$
\frac{1}{T} \sum_{t=0}^{T-1} J(\theta^t) - J(\theta^*) \leq \mathcal{O}(\frac{1}{\sqrt{T}})
$$ 

这表明，为了使平均损失与最优损失的差距减小10倍，需要的迭代次数 $T$ 大约要增加100倍。这是一种次线性 (sub-linear) 的收敛速率，相对较慢。
