...
+ 半群
  ...
  半群是一个数学概念，它是一个集合，其上定义了一个**封闭**的二元运算，这个运算满足**结合律**。【重点就是**封闭且结合律**】![]({561CC1DC-F819-4152-9E9E-10F12FF704F5} 1.png)
...
+ 同构映射：
![]({C4F358E7-398E-450B-9F33-7A2A53EFF0B2}.png)
  - 定义：
...
+ 同态映射：
  + 定义：
![]({4F65307D-3224-449C-82A8-CBCD113B0C58}.png)
...
+ ![]({3AA90FC6-EE95-4C81-8BB9-D6273AA106D5}.png)
  ...
  + 思想：设正则语言对应$NFA~~ M$
    ...
    + 消去某一中间状态：
      ...
      + 通过修改每一个前趋状态及后继状态的转移弧标记，弥补消去的转移弧
        ![](Pasted image 20251102102241.png)
        ![](Pasted image 20251102102457.png)
  + 步骤：
...
+ 状态集划分的算法-填表法
  + 填表算法：基于如下递归地标记可区别的状态偶对的过程。
    ...
    + 归纳：设p和q已标记为可区别的，若状态r和s通过输入符号a可分别转移到p和q，即$\delta(r,a)=p,\delta(s,a)=q$，则r和s也标记为可区别的。
      ![](Pasted image 20251102102533.png)
...
![](Pasted image 20251102102551.png)
...
![](Pasted image 20251102102615.png)
...
[p A r] \rightarrow a
$$
![](Pasted image 20251102103358.png)
...
- DPDA的表达能力强于于有限自动机：即
	...
	- ![](Pasted image 20250423111758.png)
#### 三. 终态型DPDA与空栈型DPDA
...
+ 消去单一产生式算法：
  ...
  + $G_1$中包含$G$的所有非单一产生式
  + ![](Pasted image 20250423121021.png)
#### 八. CFG的化简和Chomsky范式
...
![]({62B906C1-E483-4BF8-A483-0A22249686E2}.png)
![]({A81D876D-1560-4B55-B986-BAA75F6B7B60}.png)
...
![](Pasted image 20250507110851.png)
![](Pasted image 20250507110902.png)
![](Pasted image 20250507110911.png)
![](Pasted image 20250507110937.png)
![](Pasted image 20250507111021.png)
![](Pasted image 20250507111027.png)
...
+ ![](Pasted image 20251102103637.png)
...
##### 可计算函数的例子
- $f(x,y)= x+y$是可计算的![](Pasted image 20250514100947.png)![](Pasted image 20250514100956.png)
- $f(x)=2x$是可计算的![](Pasted image 20250514101057.png)![](Pasted image 20250514101108.png)
- $f(x,y)=\begin{cases} 1 &\text{if} \quad x>y \\ 0 &\text{if} \quad x\leq y\end{cases}$![](Pasted image 20250514101332.png)
- $f(x)=x^2$是可以计算的
  考虑进行$x$次加法即可
...
- 图灵机的子程序![](Pasted image 20250514102709.png)
- 计算乘法的图灵机![](Pasted image 20250514102740.png)
#### 七. 图灵理论
...
- 多通道图灵机
  多通道图灵机是一个读写头读写一个向量![](Pasted image 20250514103008.png)
- 多带图灵机![](Pasted image 20250514103323.png)
...
- 多维图灵机![](Pasted image 20250514104942.png)![](Pasted image 20250514104950.png)

对于图灵机移动的拓展有：

- 增加具有停止功能的图灵机：
	- 停止功能的TM与标准TM的功能相同![](Pasted image 20250514105654.png)
- 状态转移的拓展：非确定TM
	- ![](Pasted image 20250514110121.png)
	- 非确定TM与标准TM功能相同，对于确定TM保存所有可能的计算路径并保存在二维带中。确定TM模拟非确定TM，复杂度在指数级![](Pasted image 20251102103723.png)
	- 使用的是**广度优先搜索（BFS）**
- 半无限带自动机
...
- 计数器机
	- ![]({59B5FF53-7D04-4B4C-BF1A-4F166BA990D3}.png)
	- 可以证明：
		- 具有一个计数器的计数器机的语言接受能力相当于DPDA
		- 有两个或以上计数器的计数器机语言接受能力相当于图灵机
			- 任何递归可枚举语言可以被具有三个计数器的计数器机接受
			- ![](Pasted image 20250521101308.png)其中$X_0$为栈顶
			- ![](Pasted image 20251102103758.png)
...
- **Rice定理：递归可枚举语言的每一个非平凡性质都是不可判定的** ^52defa
	...
	- 归约的过程为，将$L_u$的实例归约到图灵机接受的语言是否满足性质$P'$。只需要从输入$(M,w)$构造$M'$即可![]({A4FB9732-ABFA-4F9D-A032-A2562CB7BE9C}.png) 
- Post对应问题：
	- 从两个长度相同的列表中选择相同顺序的项，拼接后是否能得到两个完全相同的字符串？
	  ![]({331AC037-03C0-4049-B5F5-382FADD920A6}.png)
	- Post对应问题是不可判定的